...
const SanalPOS = () => {
  ...
  const filterOptions = useMemo(() => {
    const providers = [...new Set(sanalPOSProviders.map(p => p.name.replace(' Sanal POS', '').replace(' POS', '')))];
    const posTypes = ['Sanal POS', 'E-ticaret POS', 'Online POS'];
    ...
    return {
      providers: providers.map(name => ({ 
        name, 
        count: sanalPOSProviders.filter(p => p.name.includes(name)).length 
      })),
      posTypes: posTypes.map(name => ({ 
        name, 
        count: sanalPOSProviders.length 
      })),
          ...
          <SidebarInset className="flex-1">
            <div className="p-4">
              <div className="flex items-center gap-2 mb-4">
                <SidebarTrigger />
                <h1 className="text-xl font-semibold">Sanal POS Filtreleme</h1>
              </div>

              <POSTypePage title="Sanal POS Komisyon Oranları 2025" description="E-ticaret ve online ödemeler için en uygun sanal POS çözümleri ve komisyon oranları" type="Sanal POS" Icon={Cloud} whatIsContent={
              ...
              <div className="container mx-auto px-4 py-12">
                <div className="mb-6">
                  <p className="text-sm text-gray-600">
                    Filtrelenmiş sonuçlar görüntüleniyor
                  </p>
                </div>
