...
const MobilPOS = () => {
  ...
  const [filteredProviders, setFilteredProviders] = useState(
    posProviders.filter(provider => provider.type === "Mobil (Taşınabilir) POS")
  );

  const handleFilter = (filters: FilterState) => {
    let filtered = posProviders.filter(provider => provider.type === "Mobil (Taşınabilir) POS");
    ...
    <>
      ...
      <SidebarProvider>
        <div className="min-h-screen flex w-full">
          <POSSidebar onFilter={handleFilter} posType="Mobil (Taşınabilir) POS" />
          <SidebarInset>
            <header className="flex h-16 shrink-0 items-center gap-2 px-4">
              <SidebarTrigger className="-ml-1" />
              <h1 className="text-lg font-semibold">Mobil POS Karşılaştırma</h1>
            </header>
