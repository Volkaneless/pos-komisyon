...
const MobilPOS = () => {
  ...
  const filterOptions = useMemo(() => {
    const providers = [...new Set(mobilPOSProviders.map(p => p.name.replace(' Mobil POS', '').replace(' POS', '')))];
    const posTypes = ['Mobil POS', 'Taşınabilir POS', 'Wireless POS'];
    ...
    return {
      providers: providers.map(name => ({ 
        name, 
        count: mobilPOSProviders.filter(p => p.name.includes(name)).length 
      })),
      posTypes: posTypes.map(name => ({ 
        name, 
        count: mobilPOSProviders.length 
      })),
          ...
          <SidebarInset className="flex-1">
            <div className="p-4">
              <div className="flex items-center gap-2 mb-4">
                <SidebarTrigger />
                <h1 className="text-xl font-semibold">Mobil POS Filtreleme</h1>
              </div>
              ...
              <div className="container mx-auto px-4 py-16">
                <div className="mb-6">
                  <p className="text-sm text-gray-600">
                    Filtrelenmiş mobil POS çözümleri
                  </p>
                </div>
