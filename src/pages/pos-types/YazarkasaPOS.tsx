...
const YazarkasaPOS = () => {
  ...
  const [filteredProviders, setFilteredProviders] = useState(
    posProviders.filter(provider => provider.type === "Yazar Kasa (ÖKC) POS")
  );

  const handleFilter = (filters: FilterState) => {
    let filtered = posProviders.filter(provider => provider.type === "Yazar Kasa (ÖKC) POS");
    ...
    <>
      ...
      <SidebarProvider>
        <div className="min-h-screen flex w-full">
          <POSSidebar onFilter={handleFilter} posType="Yazar Kasa (ÖKC) POS" />
          <SidebarInset>
            <header className="flex h-16 shrink-0 items-center gap-2 px-4">
              <SidebarTrigger className="-ml-1" />
              <h1 className="text-lg font-semibold">Yazar Kasa POS Karşılaştırma</h1>
            </header>
            ...
            <div>
              <POSTypePage
                ...
                advantages={[
                  "Çift fonksiyonlu kullanım (hem yazar kasa hem POS)",
                  "Yasal gerekliliklere uygunluk",
                  "Tek cihazla çoklu işlem yapabilme",
                  "Stok ve satış takibi entegrasyonu",
                  "Mali raporlamalarda kolaylık"
                ]}
              />
              ...
              <div className="container mx-auto px-4">
                ...
                <section id="karsilastirma" className="my-12">
                  ...
                  <div className="grid md:grid-cols-3 gap-6">
                    {yazarkasaPOSProviders.slice(0, 6).map(provider => <Card key={provider.id} className="hover:shadow-lg transition-shadow">
