...
const CepPOS = () => {
  ...
  const filterOptions = useMemo(() => {
    const providers = [...new Set(cepPOSProviders.map(p => p.name.replace(' Cep POS', '').replace(' POS', '')))];
    const posTypes = ['Cep POS', 'Mobil Uygulama POS', 'Android POS', 'iOS POS'];
    const commissionRanges = ['%0-1', '%1-2', '%2-3', '%3+'];
    const monthlyFees = ['Ücretsiz', '0-50 TL', '50-100 TL', '100+ TL'];

    return {
      providers: providers.map(name => ({ 
        name, 
        count: cepPOSProviders.filter(p => p.name.includes(name)).length 
      })),
      posTypes: posTypes.map(name => ({ 
        name, 
        count: cepPOSProviders.length 
      })),
          ...
          <SidebarInset className="flex-1">
            <div className="p-4">
              <div className="flex items-center gap-2 mb-4">
                <SidebarTrigger />
                <h1 className="text-xl font-semibold">Cep POS Filtreleme</h1>
              </div>
              ...
              <div className="container mx-auto px-4 py-12">
                <div className="mb-6">
                  <p className="text-sm text-gray-600">
                    Filtrelenmiş cep POS çözümleri
                  </p>
                </div>
