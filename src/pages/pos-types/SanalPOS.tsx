...
const SanalPOS = () => {
  const [filteredProviders, setFilteredProviders] = useState(
    posProviders.filter(provider => provider.type === "Sanal POS")
  );

  const handleFilter = (filters: FilterState) => {
    let filtered = posProviders.filter(provider => provider.type === "Sanal POS");
    ...
    <>
      ...
      <SidebarProvider>
        <div className="min-h-screen flex w-full">
          <POSSidebar onFilter={handleFilter} posType="Sanal POS" />
          <SidebarInset>
            <header className="flex h-16 shrink-0 items-center gap-2 px-4">
              <SidebarTrigger className="-ml-1" />
              <h1 className="text-lg font-semibold">Sanal POS Karşılaştırma</h1>
            </header>
